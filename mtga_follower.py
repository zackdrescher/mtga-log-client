...
handlers = {
    logging.handlers.TimedRotatingFileHandler(
        LOG_FILENAME, when="D", interval=1, backupCount=7, utc=True
    ),
...
LOG_START_REGEX_TIMED = re.compile(
    r"^\[(UnityCrossThreadLogger|Client GRE)\]([\d:/ .-]+(AM|PM)?)"
)
...
class Follower:
    ...
    def __retry_post(
        ...
        blob,
        num_retries=RETRIES,
        sleep_time=DEFAULT_RETRY_SLEEP_TIME,
        use_gzip=False,
    ):
        """
        ...
        while tries_left > 0:
            ...
            if not IS_CODE_FOR_RETRY(response.status_code):
                break
            logger.warning(
                f"Got response code {response.status_code}; retrying {tries_left} more times"
            )
    ...
    def __handle_ongoing_events(self, json_obj):
        ...
        response = self.__retry_post(
            f"{self.host}/{ENDPOINT_ONGOING_EVENTS}", blob=event
        )
    ...
    def __handle_bot_draft_pack(self, json_obj):
        ...
        if json_obj["DraftStatus"] == "PickNext":
            ...
            response = self.__retry_post(
                f"{self.host}/{ENDPOINT_DRAFT_PACK}", blob=pack
            )
    ...
    def __handle_human_draft_combined(self, json_obj):
        ...
        response = self.__retry_post(
            f"{self.host}/{ENDPOINT_HUMAN_DRAFT_PACK}", blob=pack
        )
        pick = {
            "player_id": self.cur_user,
            "time": self.cur_log_time.isoformat(),
        ...
        response = self.__retry_post(
            f"{self.host}/{ENDPOINT_HUMAN_DRAFT_PICK}", blob=pick
        )
    ...
    def __handle_deck_submission(self, json_obj):
        ...
        deck = {
            ...
            "sideboard_card_ids": [
                d["cardId"] for d in decks["Sideboard"] for i in range(d["quantity"])
            ],
            "companion": decks["Companions"][0]["cardId"]
        ...
        response = self.__retry_post(
            f"{self.host}/{ENDPOINT_DECK_SUBMISSION}", blob=deck
        )
    ...
    def __handle_collection(self, json_obj):
        ...
        if self.cur_user is None:
            logger.info(
                f"Skipping collection submission because player id is still unknown"
            )
            return
...
def verify_version(host, prompt_if_update_required):
    for i in range(3):
        response = requests.get(
            f"{host}/{ENDPOINT_CLIENT_VERSION}",
            params={
                "client": "python",
        ...
        logger.warning(
            "Could not get response from server for minimum client version. Assuming version is valid."
        )
        return True
